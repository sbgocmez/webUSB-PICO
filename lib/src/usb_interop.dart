...
class UsbDevice extends Delegate<Object> {
  ...
  String? get manufacturerName => getProperty("manufacturerName");

  bool? get opened => getProperty("opened");

  //bool? get connected => getProperty('connected');

  int? get deviceClass => getProperty("deviceClass");
  ...
  Future<void> controlTransferIn(ControlTransferInSetup setup, int length) {
    var promise = callMethod('controlTransferIn', [setup, length]);
    return promiseToFuture(promise);
  }

  Future<void> clearHalt(String direction, int endpointNumber) {
    var promise = callMethod('clearHalt', [direction, endpointNumber]);
    return promiseToFuture(promise);
  }
  //The method '' isn't defined for the type 'UsbDevice'.
}
...
@JS()
@anonymous
class ControlTransferInSetup {
  external factory ControlTransferInSetup({
    String requestType,
    String recipient,
    int request,
    int value,
